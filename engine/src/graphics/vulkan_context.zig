...
pub const VulkanContext = struct {
    ...
    pub fn deinit(self: *const VulkanContext) void {
    ...
    pub fn deviceName(self: *const VulkanContext) []const u8 {
    ...
    fn findMemoryTypeIndex(
        ...
        const count = self.physical_device_memory_properties.memory_type_count;
        for (self.physical_device_memory_properties.memory_types[0..count], 0..) |mem_type, i| {
            if (memory_type_bits & (@as(u32, 1) << @truncate(i)) != 0 and mem_type.property_flags.contains(flags)) {
                return @truncate(i);
            }
        }
        log.err(
            "Failed to find suitable memory type for Vulkan, bits {b} and flags {any}", 
